...
class MyMembers(commands.Cog):
    ...
    async def get_info_for_member(
        ...
        footer_text = (
            f"Generated by {bot.user} at {datetime.now().isoformat(timespec='seconds')}\n"
            f"Contain JSON informations: {get_json}"
        )
        msg_content = (
        ...
        embed = discord.Embed(
            color=member.color,
            title=f"Information for {member} (ID: {member.id})",
            description=desc,
        )
    ...
    async def cmd_get_user_info(
        ...
        if len(embeds) > 1:
            ...
            if include_json:
                total_data = {
                    "data": [],
                    "filename": f'report-from-{self.bot.user}-to-{ctx.author}-at-{datetime.now().isoformat(timespec="seconds")}.json',
                }
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send(warning(f"An unexpected error was raised: {error}"))
    ...
    async def cmd_settings(self, ctx: commands.Context):
        ...
        if config["channel"]:
            if fetched_channel := self.bot.get_channel(config["channel"]):
                embed.set_field_at(
    ...
    @mymembers.command(name="getdiff")
    async def cmd_get_diff(self, ctx: commands.Context, member: discord.Member, snowflake: int):
        """
        Check the difference of a member's original joined_at and a snowflake.
        """
        if member.joined_at > discord.utils.snowflake_time(snowflake):
            diff = member.joined_at.timestamp() - datetime.timestamp(
                discord.utils.snowflake_time(snowflake)
            )
        else:
            diff = (
                datetime.timestamp(discord.utils.snowflake_time(snowflake))
                - member.joined_at.timestamp()
            )
        await ctx.send(diff)
        await ctx.send(
            f"The difference between the member's joined_at and the snowflake you gave me is "
            f"{humanize_timedelta(seconds=diff)}."
        )
